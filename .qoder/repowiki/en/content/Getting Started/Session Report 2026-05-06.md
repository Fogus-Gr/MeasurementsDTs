...
- CPU optimization: automatic thread/stream tuning calibrated for 4-vCPU AMD EPYC 7551P cloud VM (CPU pinning disabled by design — ineffective in virtualised environments), with system-level hints (CPU governor, NUMA balancing). See [optimizations/README.md](file://optimizations/README.md) for the cloud-VM vs. bare-metal applicability table.
...
- EPIC CPU Optimizer auto-detects CPU topology (cores, AVX, L3, NUMA nodes) and selects thread/stream configuration. **CPU pinning is disabled by default** because the calibrated target is a 4 vCPU AMD EPYC 7551P cloud VM where vCPU↔pCore mapping is unstable; bare-metal deployers should flip `enable_cpu_pinning=True`.
