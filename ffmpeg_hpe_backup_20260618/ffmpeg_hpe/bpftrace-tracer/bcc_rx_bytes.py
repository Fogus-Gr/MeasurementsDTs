...
def main():
    ...
    SAMPLE_INTERVAL_MS = int(os.environ.get("BCC_SAMPLE_INTERVAL_MS", "10"))
    if SAMPLE_INTERVAL_MS <= 0:
        log("BCC_SAMPLE_INTERVAL_MS must be positive", "ERROR")
        sys.exit(1)
...
""" % (streamer_ip_int, STREAMER_PORT, HPE_PORT)
    ...
    try:
        ...
        with open(OUTPUT_CSV, "w") as f:
            ...
            log(f"Sampling RX bytes every {SAMPLE_INTERVAL_MS} ms")
            key = ctypes.c_uint(0)
            prev_bytes = 0
