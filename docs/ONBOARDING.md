...
│
├── Measure_plot_cpu_perf/         # Linux perf stat plotting tools (see [README](file:///home/lenovo/MeasurementsDTs/Measure_plot_cpu_perf/README.md))
...
> always use `requirements.txt` on `perf-tuning-base` unless doing a CPU-only setup.

> [!TIP]
> **CPU-only Installation (Without CUDA/GPU)**
> If you are deploying on a non-CUDA/CPU-only machine and want to avoid downloading large CUDA-specific wheels and `nvidia-*` packages:
> 
> 1. **Ensure Python 3.9+**: Your local environment must use **Python 3.9+** (e.g., Python 3.9.13). Do not use Python 3.8.10, as it is incompatible with newer dependencies in `requirements.txt.in` (such as `numpy>=1.26.0`).
> 2. **Conda Setup**:
>    ```bash
>    # Install CPU-only PyTorch and torchvision
>    conda install pytorch==2.4.1 torchvision==0.19.1 cpuonly -c pytorch
>    # Install remaining CPU-friendly requirements
...
### Custom FFmpeg with NVIDIA/CUDA Support

To enable hardware-accelerated video decoding/encoding on the host system, you need to compile FFmpeg with `nvenc` and `cuda` support. This provides optimal performance when running experiments locally outside of Docker.

An automated script is provided at the repository root:
```bash
./build_ffmpeg_cuda.sh
```

For a detailed step-by-step breakdown of the installation process, refer to the dedicated guide: [install_ffmpeg_nvidia.md](install_ffmpeg_nvidia.md).
...
| `Measure_plot_cpu_perf/plot_perf_metrics.py` | perf stat CSV | CPU cycles + utilization bar chart (see [README](file:///home/lenovo/MeasurementsDTs/Measure_plot_cpu_perf/README.md)) |
