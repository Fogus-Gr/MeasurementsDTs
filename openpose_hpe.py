...
class OpenPoseHPE(OpenVINOBaseHPE):
    ...
    def load_model(self, device="CPU", **kwargs):
        ...
        '''self.config = {
            'heatmaps_blob': 'Mconv7_stage2_L2',
            'nms_heatmaps_blob': 'heatmaps_nms',
            'embeddings_blob': 'Mconv7_stage2_L1',
            'decoder': 'your_decoder_function',  # Should match OpenVINO decoder API
            'output_scale': 1.0,
            'padding_mode': 'center',  # for 'ae' and 'higherhrnet'
            'model_input_shape': (self.pd_h, self.pd_w)
        }'''
    ...
    def postprocess(self, poses):
        ...
        for pose in poses:
            ...
            if score > self.score_thresh:
                ...
                # Undo padding
                #keypoints_xy_orig[:, 0] -= self.padding.w
                #keypoints_xy_orig[:, 1] -= self.padding.h
